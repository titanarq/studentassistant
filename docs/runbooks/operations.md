...
| `studentassistant-board-sync.timer` | `scripts/board_sync.py`: mirrors `status:*` labels to the Project 3 Status column (workaround, below) | 5 min |
...
ln -sf "$PWD/scripts/systemd/studentassistant-board-sync."{service,timer} ~/.config/systemd/user/
