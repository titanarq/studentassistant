...
- Find them: `issues.py list --label <labels.blocked_on_human>`, and open PR review threads or
  issue comments that mention `@MatillaM` after the label was set.
...
3. The diff (`gh pr diff N --name-only`) touches only files the issue's scope allows, none of the
   ...
   own brief (`docs/modules/workers.md` Contract, "File ownership"), only the merge-time
...
```bash
gh api -X PUT repos/titanarq/studentassistant/pulls/N/merge -f merge_method=squash -f sha=<verified head sha>
gh api -X DELETE repos/titanarq/studentassistant/git/refs/heads/<head branch>
```
...
- `systemctl --user status studentassistant-guard.timer` — a stopped timer means nothing else below is
  moving.
- `journalctl --user -u studentassistant-guard.service --since "12 hours ago" -o cat | grep -vE "never started|^0 event|nothing to wake"`.
...
took: `journalctl --user -u studentassistant-guard.service --since "10 min ago" -o cat | grep -i nudg` and a
