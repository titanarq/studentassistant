...
- The **backend**: one of the configured worktrees (qwen: ../studentassistant-qwen, claude: ../studentassistant-claude). If the caller did not say,
  ask — do not pick.
...
- Never run `scripts/test.sh` in a worktree yourself; the caller does, from that worktree, or it
