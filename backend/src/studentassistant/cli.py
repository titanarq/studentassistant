...
`cost` prints what the Claude calls recorded in the vault's ledgers cost, `setup` gets a PC from
clone to running (the vault, the Anthropic API key, the STT model, the systemd service) and
`doctor` checks that it is.
